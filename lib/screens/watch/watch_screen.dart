...
class _WatchScreenState extends State<WatchScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    if (hasExistingPlayer) {
      ...
      // ★ FIX: Đọc duration trực tiếp từ player state
      try {
        final dur = _player!.state.duration;
        if (dur.inSeconds > 0) {
          _currentDuration = dur.inSeconds;
          _currentDur = dur; // ★ FIX: Set Duration object cho UI
        } else {
          _currentDuration = PlayerHolder.currentDuration;
          _currentDur = Duration(seconds: PlayerHolder.currentDuration);
        }
      } catch (_) {
        _currentDuration = PlayerHolder.currentDuration;
        _currentDur = Duration(seconds: PlayerHolder.currentDuration);
      }
      // ★ FIX: Set currentPos Duration object cho UI
      _currentPos = Duration(seconds: _currentPosition);
  ...
  // Ad markers — proxy handles blocking
        ...
        case 'onPiPRestore':
          ...
          if (mounted) {
            ...
            if (_playerMode == _PlayerMode.hls && _player != null) {
              ...
              // ★ FIX: Play TRƯỚC rồi seek SAU — media_kit chỉ render frame khi playing
              // Nếu seek khi paused → frame không update → video hiển thị ảnh cũ
              if (!dismissed) {
                _seekCompleted = false;
                _seekTargetTime = position;
                // Play ngay để player bắt đầu render frame
                _player!.play();
                // Seek sau 100ms — player đã play, frame sẽ update đúng
